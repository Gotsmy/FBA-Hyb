...
class HYB(eqx.Module):
    ...
    def __init__(self, 
        ...
        self.SCL_yy = SCL_yy
        self.SCL_uu = SCL_uu
        self.SCL_nn = DF.SCL_ADAPTIVE_standard(dim=ANN_fba_obj.in_size)
...
def make_step(data_, model, opt_state, key, optimizer):
    ...
    new_SCL_ann = model.HYB.SCL_nn.update(mod_ann_inp_raw)
...
def get_model_config(verbose=True):
    ...
    if fba_type == "SRfba":
        ...
        if extra_metabolites:
            COL_v_ext = ["EX_co2_e","EX_nh4_e","EX_o2_e","EX_so4_e"]
        else:
            COL_v_ext = []
    ...
    TRAIN_kwargs = {
            ...
            "early_stopping"  : True,
            "disregard_first" : 500, # disregard first k epochs
...
def SLIM_preprocess_data(exp_dfs,aug_dfs,valid_set,scalers,augmented=False,verbose=True):
    if augmented:
        if verbose:
