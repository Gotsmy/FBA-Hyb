...
class HYB(eqx.Module):
    ...
    # Adaptive scaler for mod_ann_inp
    SCL_nn : eqx.Module = MF.frozen_field()
    ...
    def __init__(self, 
        ...
        self.SCL_yy = SCL_yy
        self.SCL_uu = SCL_uu
        self.SCL_nn = DF.SCL_none(dim=ANN_fba_obj.in_size)
    ...
    def get_metabolic_fluxes(self, y, Y, u, U, yi, YI, key):
        ...
        mod_ann_inp = self.SCL_nn.scale(mod_ann_inp_raw)
    ...
    def unpack_inputs(self,t, y, args):
        ...
        YI = self.SCL_yy.unscale(yi[:self.NR_state_variables])
    ...
    def dNdt_and_intermediates(self, t, y, args):
        ...
        # extra metabolites, unscaled. Scaling is not necessary as they are 
        # 1. not part of the loss and
        # 2. the mod_ann_inp is scaled with an adaptive scaler.
        dE_dt = V_ext*Xr - Tf/Y[3]*y[self.NR_state_variables:]
...
class ODE(eqx.Module):
    ...
    def get_solution(self, YT, y0, yi, UT, uc, DS, key):
        ...
        # TODO: this is somewhat a flaw in the implementation.
        # We don't know which initial values of the extra variables we want to use.
        # Here I hardcode that the last value is always sulfate.
        cutoff = yi.shape[0]-self.HYB.NR_extra_variables
        y0 = jnp.concatenate([y0, yi[cutoff:]]) #
        
        yi # concentrations of extra variables are in yi
        yi = yi[:-4] # this has to be 4 as it is hardcoded in the data preprocessing
...
def make_step(data_, model, opt_state, key, optimizer):
    ...
    new_SCL_ann = model.HYB.SCL_nn.update(mod_ann_inp_raw,alpha=.001)
    # 3. replace scaler in model
    model = eqx.tree_at(lambda m: m.HYB.SCL_nn, model, new_SCL_ann)

    # jax.debug.print("{x}", 
    #                 x=optax.global_norm(new_SCL_ann.avg))
...
def train(model, train_, valid_, train_kwargs):
    ...
    # Freeze hast to come in the end
    ...
    for i in range(train_kwargs["n_epochs"]):
        ...
        if i == 0 or i % n_prnt == 0:
                ...
                # print(optax.global_norm(inference_model.HYB.SCL_nn.avg))
...
def DO_modify_ann_inp(HYB, y, Y, u, U, yi, YI, key=None):
    mod_ann_inp = jnp.array([y[-1]])
    return mod_ann_inp
...
class StaticMLP(eqx.Module):
    """Constant-output stand-in for eqx.nn.MLP.

    Behaves like an MLP from an API/attribute perspective:
    - has `.in_size`, `.out_size`, `.width_size`, `.depth`, etc.
    - `__call__(x)` ignores `x` and returns a learned vector of shape `(out_size,)`.

    The output vector is trainable in whatever optimization loop you already use.
    """

    # ---- trainable parameters ----
    out: jax.Array

    # ---- metadata attributes (non-trainable) ----
    ...
    activation: Optional[Callable[..., Any]] = eqx.field(static=True)
    final_activation: Optional[Callable[..., Any]] = eqx.field(static=True)
    ...
    def __init__(
        ...
        *,
        ...
        final_activation: Optional[Callable[..., Any]] = None,
        use_bias: bool = True,
        ...
        **kwargs: Any,
        ...
        # store metadata so downstream code expecting MLP attributes keeps working
        self.in_size = int(in_size)
        self.out_size = int(out_size)
        self.width_size = int(width_size)
        self.depth = int(depth)
        self.use_bias = bool(use_bias)
        self.use_final_bias = bool(use_final_bias)
        self.activation = activation
        self.final_activation = final_activation
        self.dtype = dtype

        # ignore any extra kwargs meant for eqx.nn.MLP compatibility
        _ = kwargs

        # initialize trainable constant output vector
        if init == "zeros":
            self.out = jnp.zeros((self.out_size,), dtype=self.dtype)
        elif init == "normal":
            self.out = jax.random.normal(key, (self.out_size,), dtype=self.dtype) * init_scale
        else:
            raise ValueError(f"Unknown init='{init}'. Use 'zeros' or 'normal'.")

    def __call__(self, x: Any, *, key: Optional["jax.random.PRNGKey"] = None) -> jax.Array:
        # Accept and ignore x (and key) to match common Equinox call conventions.
        del x, key
        return self.final_activation(self.out)
...
def get_model_config(verbose=True):
    ...
    if fba_type == "SRfba":
        ...
        FBA_model = MF.SLIM_SRfba()

        COL_q_rel = ["EX_glc__D_e","pDNA_synthesis","BIOMASS_Ec_iML1515_core_75p37M"]
        ...
        SCL_fba_out = DF.SCL_init_from_slim_fba(COL_q_rel,method="mean")
    ...
    if base_flux_type == "none":
        ANN_base_fd = MF.RETURN_value(0.0) # always returns 0
    else:
    ...
    elif LIM_handler_type == "fixed":
        LIM_handler = MF.G_LIM_fixed
    else:
        raise NotImplementedError(f"Unknown LIM_handler_type: {LIM_handler_type}")

    ANN_fba_obj = StaticMLP(
    ...
    scalers = HF.SLIM_get_process_scalers(method="mean")
    ...
    HYB_kwargs = {
        ...
        "MOL_masses": jnp.array([180.15588, 4943.150016, 1000.]), # g/mol
        ...
        "MOD_ann_inp" : MOD_ann_inp, # function that handles the ANN input modification
        "verbose"       : False,
                 }
    ...
    TRAIN_kwargs = {
            ...
            "key_value"           : 13,
            "learning_rate"       : 1e-3,
            "processes_per_batch" : 8,
            # EARLY STOPPING HYPERPARAMS
            "early_stopping"  : True,
            "disregard_first" : 2000, # disregard first k epochs
            "max_patience"    : 250, # stop, if there is no valid loss improvement for max_patience epochs
...
def get_exp_dfs():
    exp_onl = pd.read_csv(str(REPO_ROOT / "slim_process_data" / "250818_exp_onl.csv"),index_col=0)
    exp_off = pd.read_csv(str(REPO_ROOT / "slim_process_data" / "250818_exp_off.csv"),index_col=0)
    exp_dsp = pd.read_csv(str(REPO_ROOT / "slim_process_data" / "250818_exp_dsp.csv"),index_col=0)

    return exp_off,exp_onl,exp_dsp

def get_aug_dfs():
    aug_onl = pd.read_csv(str(REPO_ROOT / "slim_process_data" / "250818_aug_onl.csv"),index_col=0)
    aug_off = pd.read_csv(str(REPO_ROOT / "slim_process_data" / "250818_aug_off.csv"),index_col=0)
    aug_dsp = pd.read_csv(str(REPO_ROOT / "slim_process_data" / "250818_aug_dsp.csv"),index_col=0)
...
if __name__ == "__main__":
    ...
    cross_valid_sets = HF.SLIM_get_valid_sets()

    # generate path and dir
    save_path = str(REPO_ROOT / "slim_model_data" / Path(__file__).stem) + "/"
    ...
    for nr, valid_set in enumerate(cross_valid_sets):
        ...
        ALL_data = SLIM_preprocess_data(exp_dataframes,aug_dataframes,valid_set,scalers,augmented=False)
        ...
        HF.print_and_write_CV_stats(SC_y_true,SC_best_preds,SC_last_preds,tested_names,save_path,id="_sc",verbose=True)
        HF.print_and_write_CV_stats(US_y_true,US_best_preds,US_last_preds,tested_names,save_path,id="_us",verbose=False)
