...
def build_model(qG, nr_X, nr_P, nr_M, tmp_model):
    ...
    snek.set_bounds(tmp_model,"EX_glyc_e_i",qG,qG)
    return tmp_model

def load_model():
    # load model
    model = cobra.io.read_sbml_model("/mnt/y/code/250513_metabolic_process_models/prol_fba_data/250414_iEC1356_reduced.xml")
    model.solver = "cplex"


    # replace glucose by glycerol as main C source
    # snek.set_bounds(model,"EX_glc__D_e_i",0,0)
    snek.set_bounds(model,"EX_glyc_e_i",0,10)
    ...
    model.reactions.BIOMASS_Ec_iJO1366_core_53p95M.add_metabolites({xxx_equivalent:1})
    model.reactions.ATPM.add_metabolites({atp_equivalent:1})
    model.reactions.protein_synthesis.add_metabolites({pro_equivalent:1})
...
if __name__ == "__main__":
    ...
    scaled_sample = qmc.scale(norm_sample,[0,0,0,0],[20,1,0.01,100])
    ...
    df.to_csv("/mnt/y/code/250513_metabolic_process_models/prol_fba_data/250721_obj_fba_01.csv")
    print("done")
